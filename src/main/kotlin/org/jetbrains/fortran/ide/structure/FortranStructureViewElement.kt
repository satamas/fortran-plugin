...
class FortranStructureViewElement(
    ...
    override fun getPresentation(): ItemPresentation = when (psi) {
        ...
        // program units
        is FortranMainProgram -> PresentationData(
            psi.programStmt?.text ?: "Program",
            null,
            FortranIcons.mainProgramIcon,
            null
        )

        is FortranFunctionSubprogram -> PresentationData(psi.functionStmt.text, null, FortranIcons.functionIcon, null)
        is FortranSubroutineSubprogram -> PresentationData(
            psi.subroutineStmt.text,
            null,
            FortranIcons.subroutineIcon,
            null
        )
        ...
        is FortranSeparateModuleSubprogram -> PresentationData(
            psi.mpSubprogramStmt.text,
            null,
            FortranIcons.separateModuleSubprogramIcon,
            null
        )
        ...
        // types
        is FortranDerivedTypeDef -> PresentationData(
            psi.derivedTypeStmt.typeDecl.text,
            null,
            FortranIcons.typeIcon,
            null
        )
    ...
    override fun getChildren() = childElements
        .sortedBy { it.textOffset }
            .map(::FortranStructureViewElement)
            .toTypedArray()

    override fun navigate(requestFocus: Boolean) = psi.navigate(requestFocus)

    override fun canNavigate() = psi.canNavigate()

    override fun canNavigateToSource() = psi.canNavigateToSource()
        ...
        get() {
            return when (psi) {
                ...
                is FortranProgramUnit -> {
                    val subprograms = psi.subprograms.filter { it.parent is FortranBeginUnitStmt }
                        .map { it.parent.parent as NavigatablePsiElement }
                    val types = psi.types.map { it.parent.parent as NavigatablePsiElement }
                    val interfaces = PsiTreeUtil.getChildrenOfType(
                        (psi as FortranProgramUnitImpl).getBlock(),
