...
class FortranContinueInspection : LocalInspectionTool() {
    ...
    companion object {
        fun createFix(continueStmt: FortranContinueStmt) : LocalQuickFix {
            ...
            return if (firstStmt != null && lastStmt != null)
                SubstituteTextFix(firstStmt.smartPointer(), lastStmt.smartPointer(),
                        "", "Continue statement without label fix"
                )
            else {
                val lastElement = if (continueStmt.nextSibling?.node?.elementType != TokenType.WHITE_SPACE)
                    continueStmt
                else
                    continueStmt.nextSibling
                SubstituteTextFix(continueStmt.smartPointer(), lastElement.smartPointer(),
                        "", "Continue statement without label fix"
                )
            }
        }
